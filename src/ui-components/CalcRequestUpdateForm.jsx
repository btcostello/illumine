...
export default function CalcRequestUpdateForm(props) {
  const {
    id: idProp,
    calcRequest: calcRequestModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = calcRequestRecord
      ? { ...initialValues, ...calcRequestRecord }
      : initialValues;
    setAge(cleanValues.Age);
    setSex(cleanValues.Sex);
    setTobacco(cleanValues.Tobacco);
    setCreditMethod(cleanValues.CreditMethod);
    setFaceAmount(cleanValues.FaceAmount);
    setAnnualPrem(cleanValues.AnnualPrem);
    setPremYrs(cleanValues.PremYrs);
    setErrors({});
  };
  const [calcRequestRecord, setCalcRequestRecord] =
    React.useState(calcRequestModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(CalcRequest, idProp)
        : calcRequestModelProp;
      setCalcRequestRecord(record);
    };
    queryData();
  }, [idProp, calcRequestModelProp]);
  React.useEffect(resetStateValues, [calcRequestRecord]);
  const validations = {
    Age: [],
    Sex: [],
    Tobacco: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            CalcRequest.copyOf(calcRequestRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "CalcRequestUpdateForm")}
      {...rest}
    >
      <TextField
        label="Age"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || calcRequestModelProp)}
