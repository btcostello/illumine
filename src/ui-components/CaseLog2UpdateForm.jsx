...
export default function CaseLog2UpdateForm(props) {
  const {
    id: idProp,
    caseLog2: caseLog2ModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = caseLog2Record
      ? { ...initialValues, ...caseLog2Record }
      : initialValues;
    setUser(cleanValues.user);
    setFaceAmount(cleanValues.faceAmount);
    setProd(cleanValues.prod);
    setSex(cleanValues.sex);
    setSmoker(cleanValues.smoker);
    setCaseName(cleanValues.caseName);
    setCaseDesc(cleanValues.caseDesc);
    setErrors({});
  };
  const [caseLog2Record, setCaseLog2Record] = React.useState(caseLog2ModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(CaseLog2, idProp)
        : caseLog2ModelProp;
      setCaseLog2Record(record);
    };
    queryData();
  }, [idProp, caseLog2ModelProp]);
  React.useEffect(resetStateValues, [caseLog2Record]);
  const validations = {
    user: [],
    faceAmount: [],
    prod: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            CaseLog2.copyOf(caseLog2Record, (updated) => {
      ...
      {...getOverrideProps(overrides, "CaseLog2UpdateForm")}
      {...rest}
    >
      <TextField
        label="User"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || caseLog2ModelProp)}
